window.renderProveedores = function (container) {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    const url = editId ? 'http://localhost:3000/api/fernandosandoval/proveedores/' + editId : 'http://localhost:3000/api/fernandosandoval/proveedores';
  ...
  async function loadTabla() {
    ...
    const res = await fetch('http://localhost:3000/api/fernandosandoval/proveedores');
    ...
    el.querySelectorAll('.edit').forEach((b) => b.addEventListener('click', async () => {
      ...
      const res = await fetch('http://localhost:3000/api/fernandosandoval/proveedores/' + id);
      if (res.status === 200) {
        const data = await res.json();
    ...
    el.querySelectorAll('.del').forEach((b) => b.addEventListener('click', async () => {
      ...
      const id = b.dataset.id;
      const res = await fetch('http://localhost:3000/api/fernandosandoval/proveedores/' + id, { method: 'DELETE' });
      if (res.status === 204) {
