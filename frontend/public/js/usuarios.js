window.renderUsuarios = function (container) {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    const url = editId ? 'http://localhost:3000/api/anthonymorales/usuarios/' + editId : 'http://localhost:3000/api/anthonymorales/usuarios';
  ...
  async function loadLista() {
    ...
    el.querySelectorAll('.edit').forEach((b) => b.addEventListener('click', async () => {
      ...
      const res = await fetch('http://localhost:3000/api/anthonymorales/usuarios/' + id);
      if (res.status === 200) {
        const data = await res.json();
    ...
    el.querySelectorAll('.del').forEach((b) => b.addEventListener('click', async () => {
      ...
      const id = b.dataset.id;
      const res = await fetch('http://localhost:3000/api/anthonymorales/usuarios/' + id, { method: 'DELETE' });
      if (res.status === 204) {
