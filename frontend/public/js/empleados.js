window.renderEmpleados = function (container) {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      const url = editId ? 'http://localhost:3000/api/davidmoran/empleados/' + editId : 'http://localhost:3000/api/davidmoran/empleados';
  ...
  async function loadTabla() {
    ...
    const res = await fetch('http://localhost:3000/api/davidmoran/empleados');
    ...
    el.querySelectorAll('.edit').forEach((b) => b.addEventListener('click', async () => {
      ...
      const res = await fetch('http://localhost:3000/api/davidmoran/empleados/' + id);
      if (res.status === 200) {
        const data = await res.json();
    ...
    el.querySelectorAll('.del').forEach((b) => b.addEventListener('click', async () => {
      ...
      const res = await fetch('http://localhost:3000/api/davidmoran/empleados/' + id, { method: 'DELETE' });
      if (res.status === 204) {
